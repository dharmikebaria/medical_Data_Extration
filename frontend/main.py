    ...
    with col2:
        if st.session_state:
            ...
            if file_format == "prescription":
                ...
                refill = st.text_input(label="Refill", value=st.session_state["refills"])
            elif file_format == "patient_details":
                name = st.text_input(label="Name", value=st.session_state["patient_name"])
                phone = st.text_input(label="Phone No.", value=st.session_state["phone_number"])
                vacc_status = st.text_input(label="Hepatitis B vaccination status", value=st.session_state["hepatitis_b_vaccination"])
                med_problems = st.text_input(label="Medical Problems", value=st.session_state["medical_problems"])
                has_insurance = st.text_input(label="Does patient have taken insurance?", value="N/A")
